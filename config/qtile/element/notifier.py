...
class Notifier(configurable.Configurable):
    ...
    async def _configure(self) -> None:
        ...
        for win in range(self.max_windows):
            ...
            self._positions.append((self.x, self.y + win * (self.height + 2 * self.border_width + self.gap)))

        # Clear defunct callbacks left when reloading the config
        notifier.callbacks.clear()
        notifier.close_callbacks.clear()

        await notifier.register(self._notify, Notifier.capabilities, on_close=self._on_close)
    ...
    def _send(self, notif: Notification, popup: Popup, timeout: Optional[int] = None) -> None:
        """
    ...
    def _load_icon(self, notif: Notification) -> Optional[Tuple[ImageSurface, int]]:
        ...
        try:
            ...
            surface, _ = images._decode_to_image_surface(img.bytes_img, img.width, img.height)
