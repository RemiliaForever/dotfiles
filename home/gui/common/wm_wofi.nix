{ pkgs, ... }:

{
  programs.wofi = {
    enable = true;
    settings = {
      show = "drun";
      sort_order = "alphabetical";
      matching = "contains";
      no_actions = true;
      always_parse_args = true;
      show_all = true;
      print_command = true;
      layer = "overlay";
      insensitive = true;
      prompt = "Search...";
      allow_markup = true;
      allow_images = true;
    };
    style = ''
      @define-color bg #161320;
      //@define-color bg-hover #1E1E2E;
      //@define-color blue #96CDFB;
      //@define-color sky #89DCEB;
      //@define-color red #F28FAD;
      //@define-color pink #F5C2E7;
      @define-color lavender #C9CBFF;
      //@define-color rosewater #F5E0DC;
      @define-color flamingo #F2CDCD;
      @define-color fg #D9E0EE;
      //@define-color green #ABE9B3;
      @define-color dark-fg #161320;
      //@define-color peach #F8BD96;
      //@define-color border @blue;
      //@define-color gray2 #6E6C7E;
      //@define-color black4 #575268;
      //@define-color black3 #302D41;
      //@define-color maroon #E8A2AF;

      * {
        font-family: "Victor Mono Nerd", "Noto Sans CJK SC Regular", "sans";
        font-size: 16px;
      }

      #window {
        margin: 0px;
        background: unset;
      }

      #outer-box {
        margin: 15px;
        padding: 10px;
        border-radius: 7px;
        background-color: inherit;
        border-radius: 7px;
        background: linear-gradient(
          90deg,
          rgba(30, 30, 46, 0.9) 26%,
          rgba(26, 24, 38, 0.9) 55%,
          rgba(22, 19, 32, 0.9) 91%
        );
        box-shadow: 0 0 5px 5px @bg;
      }

      #input {
        border: 2px solid @flamingo;
        border-radius: 10px;
        color: @lavender;
        background: rgb(30, 30, 46);
        background: linear-gradient(
          90deg,
          rgba(30, 30, 46, 1) 43%,
          rgba(26, 24, 38, 1) 82%,
          rgba(22, 19, 32, 1) 91%
        );
      }

      #input:focus {
        background: rgb(48, 45, 65);
        background: linear-gradient(
          90deg,
          rgba(48, 45, 65, 1) 19%,
          rgba(30, 30, 46, 1) 77%,
          rgba(26, 24, 38, 1) 100%
        );
      }

      #input:focus image {
        color: @flamingo;
      }

      #input image {
        color: @lavender;
      }

      #scroll {
        margin-top: 10px;
      }

      #inner-box {
        border-radius: 7px;
      }

      #text {
        margin: 5px;
        border: none;
        color: @fg;
      }

      #entry:selected {
        border-radius: 10px;
        background: rgb(201, 203, 255);
        background: linear-gradient(
          90deg,
          rgba(201, 203, 255, 1) 28%,
          rgba(245, 224, 220, 1) 85%,
          rgba(242, 205, 205, 1) 100%
        );
      }

      #text:selected {
        background-color: inherit;
        color: @dark-fg;
        font-weight: normal;
      }
    '';
  };
}
