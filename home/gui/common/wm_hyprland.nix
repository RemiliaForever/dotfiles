...
{
  options = {
    wayland.windowManager.hyprland.autoStart = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = "Whether to auto start hyprland in graphical session.";
    };
  };
  ...
  config = {
    ...
    programs.zsh.loginExtra =
      if config.wayland.windowManager.hyprland.autoStart then
        ''
          # UWSM auto start
